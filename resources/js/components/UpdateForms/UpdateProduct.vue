            ...
            <div class="formContainer">
                <form>
                    ...
                    <div v-for="attribute in attributes" :key="attribute.id">
                        ...
                        <br />
                        {{ getAttributeDataIfFilled(attribute.id, 'price') }}
                        <label for="">قیمت:</label>
                        <input type="text" :name="`product_attributes[${attribute.id}][price]`" :value="getAttributeDataIfFilled(attribute.id, 'price')" ref="product_attributes">
                        <div v-for="error in getAttributeErrors(attribute.id, 'price')" style="color : red">
                            {{ error }}
                        </div>
                        <label for="">تعداد:</label>
                        <input type="text" :name="`product_attributes[${attribute.id}][stock]`" :value="getAttributeDataIfFilled(attribute.id, 'stock')" ref="product_attributes">
                        ...
                        <select :name="`product_attributes[${attribute.id}][discount_id]`" ref="product_attributes">
                            <option v-if="getAttributeDataIfFilled(attribute.id, 'discount_id').length == 0" value="">بدون تخفیف</option>
                            <option v-for="discount in discounts" :value="`${discount.id }`" :selected="discount.id === getAttributeDataIfFilled(attribute.id, 'discount_id')">{{ discount.value }}%</option>
                        </select>
                    </div>
                    <div
                        style="color: red"
